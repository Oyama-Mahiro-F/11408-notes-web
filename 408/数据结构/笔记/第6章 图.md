...
> **408 高频考点提醒**：
> - **握手定理**：无向图 $\sum\deg(v)=2e$；有向图 $\sum\text{in}=\sum\text{out}=e$
> - **邻接矩阵**：第 $i$ 行非零数=$v_i$ 度（有向第 $i$ 行为出度、第 $i$ 列为入度）；空间 $O(n^2)$ 适稠密
> - **DFS/BFS**：DFS 用**栈**、BFS 用**队列**；邻接表均 $O(n+e)$、邻接矩阵 $O(n^2)$
> - **Prim/Kruskal**：Prim **加点** $O(n^2)$ 适稠密；Kruskal **加边**+并查集 $O(e\log e)$ 适稀疏
> - **Dijkstra/Floyd**：Dijkstra 单源**递增次序**、**禁负权** $O(n^2)$；Floyd 多源 $O(n^3)$、**禁负环**
> - **拓扑排序**：反复输出**入度 0** 顶点并删弧（入度 $-1$），$O(n+e)$；能输出全部顶点则**无环**
> - **关键路径**：顶点=事件、边=活动，工期=**最长路径**；$e(i)=l(i)$（松弛量为 0）即**关键活动**
> - **生成树**：极小连通子图恰 $n-1$ 边；边数 $<n-1$ 非连通、$>n-1$ 必有环、$n-1$ 边≠必生成树
...
| # | 性质 | 公式 |
|:---|:---|:---|
| 1 | 无向图总度数 = 边数的两倍 | $\sum \deg(v) = 2\vert E\vert$ |
| 2 | 有向图入度之和 = 出度之和 | $\sum \text{in}(v) = \sum \text{out}(v) = \vert E\vert$ |
| 3 | 完全图边数（无向） | $n(n-1)/2$ |
| 4 | 完全图边数（有向） | $n(n-1)$ |
| 5 | 连通图最少边数 | $n-1$（生成树） |
...
└────────┴──────────┘         └──────┴──────┴──────┘
```
图：邻接表结点结构——顶点表结点含 data（顶点数据）与 firstarc（出边链头指针）；边表结点含 adjvex（邻接点下标）、weight（权值）、next（指向下一条边）。
...
```
vertex node (VexBox)                     edge node (EBox)              
┌────────┬──────────┐       ┌──────┬──────┬──────┬──────┬──────┬──────┐
│  data  │firstedge │──→    │ mark │ ivex │ilink │ jvex │jlink │ info │
└────────┴──────────┘       └──────┴──────┴──────┴──────┴──────┴──────┘
```
图：邻接多重表结点结构——顶点结点含 data 与 firstedge（第一条依附边）；边结点含 mark（访问标志）、ivex/jvex（两端顶点下标）、ilink/jlink（各自的下一条依附边）、info（权值）。
...
```
   vertex node (VexNode)                     arc node (ArcBox)             
┌──────┬─────────┬─────────┐    ┌──────┬──────┬──────┬──────┬──────┬──────┐
│ data │firstout │ firstin │    │tailvex│headvex│hlink│tlink│ info │      │
└──────┴─────────┴─────────┘    └──────┴──────┴──────┴──────┴──────┴──────┘
```
图：十字链表结点结构——顶点结点含 data、firstout（第一条出弧）、firstin（第一条入弧）；弧结点含 tailvex（弧尾下标）、headvex（弧头下标）、hlink（同弧头的下一条弧）、tlink（同弧尾的下一条弧）、info（权值）。
...
[DFS 深度优先搜索 演示](第6章 图.assets/dfs.html)
...
[BFS 广度优先搜索 演示](第6章 图.assets/bfs.html)
...
> **生成树四结论**（教材页 67）——无向图生成树的四个判断要点：
> 1. 一棵含 $n$ 个顶点的生成树**有且仅有 $n-1$ 条边**。
> 2. 若图有 $n$ 个顶点而边数**小于 $n-1$**，则必为**非连通图**。
> 3. 若边数**多于 $n-1$**，则图**一定有环**。
> 4. **有 $n-1$ 条边的图不一定是生成树**（可能含环、可能非连通）。
...
[Prim 算法 演示](第6章 图.assets/prim.html)
...
[Kruskal 算法 演示](第6章 图.assets/kruskal.html)
...
[Dijkstra 最短路径 演示](第6章 图.assets/dijkstra.html)
...
[Floyd 多源最短路径 演示](第6章 图.assets/floyd.html)
...
[拓扑排序 演示](第6章 图.assets/topological-sort.html)
...
| 活动 | 边 | 时长 | e(i)=ve(起点) | l(i)=vl(终点)-时长 | 松弛量 l(i)-e(i) | 是否关键 |
|:---|:---|:---:|:---:|:---:|:---:|:---:|
| a1 | ①→② | 3 | 0 | 5-3=2 | 2 | 非关键 |
| a2 | ②→④ | 2 | 5 | 7-2=5 | 0 | **关键** |
| a3 | ①→③ | 4 | 0 | 4-4=0 | 0 | **关键** |
...
*关键路径：①→③→②→④（长度=7天）；关键活动 a2、a3、a4（松弛量为 0，一天不能延迟），非关键活动仅 a1（松弛量 2 天）。*

> **缩短关键路径**（教材页 94）：要缩短工期必须缩短**关键活动**的持续时间；若存在**多条关键路径**，则须**同时缩短所有关键路径**，否则工期不变。关键活动也不能无限缩短——当它缩短到某一程度时，原有非关键活动会上升为关键活动，继续缩短将受限于新出现的关键路径。

[关键路径 演示](第6章 图.assets/critical-path.html)
...
```
第6章 图
├─（一）图的基本概念（完全图 n(n-1)/2、握手定理 Σdeg=2e、连通/强连通分量、生成树 n-1 边）
├─（二）图的存储结构（邻接矩阵 O(n²) 稠密 / 邻接表 O(n+e) 稀疏 / 邻接多重表 / 十字链表）
├─（三）图的遍历（DFS 栈 / BFS 队列，均 O(n+e)、矩阵 O(n²)；生成树与森林 n-k 边）
├─（四）最小生成树（生成树四结论 n-1 边 / <n-1 非连通 / >n-1 有环 / n-1 边≠必生成树；Prim 加点 O(n²) / Kruskal 加边 O(e log e)；唯一性判定）
├─（五）最短路径（Dijkstra O(n²) 禁负权 / Floyd O(n³) 禁负环 / Bellman-Ford）
├─（六）拓扑排序（AOV 入度 0 输出删弧、O(n+e)、环检测）
└─（七）关键路径（AOE ve/vl 递推、e(i)=ve、l(i)=vl-时长、e==l 关键活动；缩工期须缩关键活动、多关键路径须同缩）
```
...
| 考点 | 一句话记忆 |
|:---|:---|
| **完全图** | 无向 $n(n-1)/2$、有向 $n(n-1)$ 条边 |
| **握手定理** | 无向 $\sum\deg(v)=2e$；有向 $\sum\text{in}=\sum\text{out}=e$ |
| **连通/强连通** | 连通图任意两点有路径；强连通图任意两点**双向可达** |
| **生成树** | 极小连通子图恰 $n-1$ 条边；生成森林总边数 $n-k$ |
| **生成树判断** | 恰 $n-1$ 边是生成树；边数 $<n-1$ 非连通、$>n-1$ 有环；有 $n-1$ 边≠必生成树 |
| **邻接矩阵** | 空间 $O(n^2)$、判边 $O(1)$，适合**稠密图**；第 $i$ 行非零数=度 |
| **邻接表** | 空间 $O(n+e)$、判边 $O(\deg)$，适合**稀疏图**；无向结点 $2e$、有向 $e$ 个 |
| **邻接多重表** | 无向图每条边**只存一次**，边操作高效，空间 $O(n+e)$ |
| **十字链表** | 有向图：tlink 沿出弧、hlink 沿入弧，出度入度都高效 |
| **DFS** | 用**栈**（递归），类比先序遍历，邻接表 $O(n+e)$、矩阵 $O(n^2)$ |
| **BFS** | 用**队列**，类比层序遍历，可求**无权图最短路径** |
| **生成树边类型** | DFS 树只有**回边**；BFS 树只有**横跨边**；DFS 树高 > BFS 树 |
| **遍历调用次数** | 等于连通分量数 $k$；连通图只调用 1 次 |
| **遍历唯一性** | 邻接矩阵结果**唯一**；邻接表不唯一（依赖链表顺序） |
...
| **缩短工期** | 缩短工期须缩**关键活动**（松弛量 0）；多条关键路径须**同时缩短**，且不能无限缩短 |
