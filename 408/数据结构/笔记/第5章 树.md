...
> - **堆**：完全二叉树 + 大根堆/小根堆；建堆 O(n)（从 ⌊n/2⌋ 下坠），插入/删除 O(log n)；堆排序见第8章
...
> 堆（Heap）是 408 考纲"（四）树-堆"明确考点（教材 5.4 / 王道口径）。堆本质上是一棵**顺序存储的完全二叉树**，利用完全二叉树"父结点下标 $i$ 的左右孩子分别为 $2i$、$2i+1$"的性质，用数组即可实现。

#### 一、堆的定义

##### 1. 大根堆与小根堆

* **大根堆（Max Heap）**：任一结点的值都 **≥** 其左右孩子的值。用数组 $a$（下标从 1 开始）表示时，对 $i=1,2,\dots,\lfloor n/2\rfloor$ 恒有 $a_i \ge a_{2i}$ 且 $a_i \ge a_{2i+1}$。
* **小根堆（Min Heap）**：任一结点的值都 **≤** 其左右孩子的值，即 $a_i \le a_{2i}$ 且 $a_i \le a_{2i+1}$。

> **核心结论**：大根堆的**堆顶（根 $a_1$）是整棵完全二叉树的最大值**；小根堆的堆顶是最小值。堆只保证**父子有序**，不保证整体有序（左右子树之间无大小约束）。

##### 2. 存储与性质

* 完全二叉树 → 顺序存储（数组），无空间浪费；双亲 $i$ 的左孩子 $2i$、右孩子 $2i+1$，双亲 $\lfloor i/2\rfloor$（与第（二）节完全二叉树编号一致）。
* 堆是**局部有序**结构，不支持像 BST 那样 $O(\log n)$ 的任意查找。

#### 二、建堆（向下调整 / 下坠）

##### 1. 向下调整 SiftDown

将某结点"下沉"到合适位置：比较该结点与左右孩子，若小于（大根堆）/ 大于（小根堆）孩子中的较大/较小者，则与**最值孩子交换**，重复直到满足堆性质或无孩子。单次 SiftDown 时间复杂度 **$O(\log n)$**（最坏下坠一层）。

##### 2. 建堆过程

* 从**最后一个非终端结点**（即下标 $\lfloor n/2\rfloor$ 处）开始，自底向上对每个结点反复执行 SiftDown，直到根结点。
* 建堆总复杂度 **$O(n)$**（看似 $\lfloor n/2\rfloor \times O(\log n)$，但底层大量结点深度小，数学求和为 $O(n)$）。

**小例：序列 [4, 10, 3, 5, 1] 建大根堆（根结点一轮下坠示意，1-based 下标）**

```
initial array: [4, 10, 3, 5, 1]   (1-based index)

        [4]            <- root (index 1), violate: 4 < max(10,3)
       /   \
    [10]   [3]         <- children of root (index 2,3)
    /  \
  [5]  [1]             <- children of [10] (index 4,5)

sift-down root [4]:
  compare 4 with children 10,3 -> larger child is 10 at index 2
  4 < 10 -> swap [4] <-> [10]
  ...
  now 4 at index 2, children 5,1 -> 4 < 5 -> swap [4] <-> [5]
  ...
  4 at index 4 has no children -> stop
  max-heap built: [10, 5, 3, 4, 1]
```

> 图：对根结点 4 执行一次向下调整（下坠）建大根堆——与较大孩子反复交换直到落位，最终堆顶为最大值 10。

#### 三、插入与删除（上浮 / 下坠）

##### 1. 插入（向上调整 SiftUp）

* 将新元素放在**数组表尾（完全二叉树最右下）**，然后自底向上与其双亲比较。
* 大根堆中若新元素 **>** 双亲则交换（上浮），直到满足堆性质或到根。
* 单次插入时间复杂度 **$O(\log n)$**。

##### 2. 删除堆顶（出堆）

* 不能直接删根（会破坏完全二叉树结构），做法是：用**表尾元素顶替堆顶**，删除表尾，再对新的堆顶执行 **SiftDown（下坠）**。
* 单次删除（出堆顶）时间复杂度 **$O(\log n)$**。

#### 四、堆与排序的关系

* 反复"删除堆顶（取最大值/最小值）"即可得到一个有序序列——这就是**堆排序**的思想（建堆 $O(n)$ + $n$ 次 $O(\log n)$ 出堆）。
* **堆排序的完整算法与稳定性分析见第 8 章（排序）**，本章只掌握堆的定义与建堆/插入/删除的基本操作。
...
├─（四）堆
│   ├─ 定义：完全二叉树 + 大根堆(ai≥a2i,ai≥a2i+1)/小根堆
│   ├─ 建堆：从 ⌊n/2⌋ 起反复下坠，O(n)
│   ├─ 插入/删除：表尾上浮/下坠，均 O(log n)
│   └─ 呼应：堆排序应用见第8章
├─（五）哈夫曼树
...
| 堆定义 | 顺序存储的**完全二叉树**；大根堆 **aᵢ ≥ a₂ᵢ 且 aᵢ ≥ a₂ᵢ₊₁**（堆顶=最大值），小根堆相反 |
| 建堆 | 从**最后一个非终端 ⌊n/2⌋** 起自底向上**下坠**，总复杂度 **O(n)** |
| 堆插入 | 放表尾后**上浮**（与双亲比较交换），O(log n) |
| 堆删除（出堆顶） | 表尾顶替堆顶后**下坠**，O(log n) |
| 堆与排序 | 反复出堆顶得有序序列=**堆排序**，完整算法见**第 8 章** |
