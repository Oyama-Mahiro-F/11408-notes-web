...
> **408 高频考点提醒**：
> - **串的定义**：零个或多个**字符**组成的有限序列；空串 `""` 长度 0，空格串长度≥1，二者不同。
> - **子串与位置**：子串是主串中任意**连续**字符子序列；字符位置**从 1 开始**。
> - **定长顺序存储**：`ch[MAXLEN+1]`，超过 MAXLEN 被**截断**；区分**非紧缩/紧缩格式**（存储密度）。
> - **BF（朴素）匹配**：失配时主串指针 **i 回溯**重试；最好 $O(n)$、最坏 **$O(nm)$**，空间 $O(1)$。
> - **next 数组求法**：$next[j]=\max\{k\mid 1<k<j$ 且 $T[1..k-1]=T[j-k+1..j-1]\}$；$next[1]=0$，无公共前后缀为 1。
> - **nextval 改进**：若 $T[i]=T[next[i]]$ 则 $nextval[i]=nextval[next[i]]$，**跳过必然再次失配**的回溯。
> - **KMP 复杂度**：主串指针 **i 永不回溯**，匹配时间 $O(n+m)$、空间 $O(m)$（next 数组）。
> - **KMP vs BF**：BF 主串回溯、最坏 $O(nm)$；KMP $i$ 不回溯、$O(n+m)$；模式串短用 BF、长用 KMP。
...
**非紧缩格式 vs 紧缩格式（顺序串存储密度）**：顺序串把字符依次存进连续单元。**非紧缩格式**——每个存储单元只存一个字符（存储密度小、较浪费空间，但处理单个字符方便、实现简单）；**紧缩格式**——每个存储单元打包多个字符（存储密度大、省空间，但需从同一单元中分离字符，运算效率低、实现复杂）。本讲主要讨论非紧缩格式（直观、易实现）。
...
图：串 "abc" 的链式存储——每个结点存放一个字符（数据域），指针域（*）指向下一个结点，末结点指针为 NULL。
...
├── 基本概念（空串/空格串/子串/位置）
├── 存储结构
│   ├── 定长顺序存储（截断风险；非紧缩/紧缩格式）
│   ├── 堆分配存储（动态长度）
│   ├── 常规链存储（每结点 1 字符，存储密度低）
│   └── 块链存储（每结点多字符，存储密度）
├── 模式匹配
│   ├── BF 算法（O(nm) / i 回溯）
│   └── KMP 算法（O(n+m) / i 不回溯）
...
| 考点 | 一句话记忆 |
|:---|:---|
| 串定义 | 零个或多个**字符**组成的有限序列；串长 $n\geq 0$，$n=0$ 为空串 |
| 空串 vs 空格串 | **空串** `""` 长度 0；**空格串**由一个或多个空格组成，长度≥1，二者不同 |
| 串相等 | 长度相等 **且** 对应位置字符**一一相等**（"abc"≠"aBc"、""≠" "） |
| 子串与位置 | 子串=主串中任意**连续**字符组成的子序列；字符位置**从 1 开始** |
| 串 vs 线性表 | 串元素是字符、操作对象是**整体（子串）**、主要操作是子串定位/替换/连接 |
| 基本操作-Index | Index(S,T) 返回 T 在 S 中**首次出现位置**（≥1），不存在返回 0 |
| 基本操作-越界 | SubString/StrInsert/StrDelete 越界返回 0；Replace 返回替换次数 |
| 定长顺序存储 | `ch[MAXLEN+1]`，超过 MAXLEN 的部分被**截断** |
| 非紧缩 vs 紧缩 | **非紧缩**=每字存 1 字符（密度小、处理方便、实现简单）；**紧缩**=每字多字符（密度大、省空间、分离字符效率低） |
| 常规链存储 | 每结点存 1 个字符，**存储密度低**（指针占 4~8 字节） |
| 堆分配存储 | `malloc/free` 动态分配，长度**可动态增长** |
| 块链存储 | 每结点存多个字符（块）；存储密度=串值字节/结点总字节 |
| BF 算法 | 失配时主串指针 **i 回溯**重试；最好 $O(n)$、最坏 **$O(nm)$**，空间 $O(1)$ |
| KMP 思想 | **i 永不回溯**，失配时 j 跳到 next[j]，利用模式串自身结构；匹配时间 **$O(n+m)$** |
| 前缀/后缀 | 前缀=除末字符外所有**头部**子串；后缀=除首字符外所有**尾部**子串 |
| 最长公共前后缀 | 前缀与后缀集合交集中**最长**者（如 "abab" 为 2） |
...
| KMP 适用 | 模式串**长**、字符集小用 KMP；模式串短用 BF |
| Get_Next 算法 | 双指针递推：相等则 i++、j++ 且 next[i]=j；不等则 j=next[j] |
| 匹配问题定义 | 主串长 $n$、模式串长 $m$（$m\leq n$）；成功返回起始位置，失败返回 0 |
| 模式匹配流程 | 先**求 next（nextval）**再匹配；nextval 在字符重复多、失配频繁时更快 |
| 串操作对象 | 线性表操作**单个元素**（查找/插入/删除）；串操作**子串整体**（定位/替换/连接） |
