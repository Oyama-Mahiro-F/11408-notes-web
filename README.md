...
- **首页 / 目录索引**（照搬旧站 idx 逻辑）：首页卡片与顶部标签进入科目索引页（面包屑 + 「N 个目录 · N 个章节」+ 卡片网格），逐层下钻到章节；目录卡显示章节数，文件卡显示科目图标
- 三栏布局：顶栏标签 / 左侧目录树（可收起成细条、自动展开定位）/ 右侧页内 TOC（可收起；层级树默认只到节标题，点箭头展开、滚动时自动展开到当前阅读位置，打开笔记时默认展开）
- **思维导图**：`思维导图大纲.md` 用 markmap 渲染（缩放/平移/折叠，公式也支持）；大纲文件为仓库内源文件，`sync.py` 同步时自动保留
- **搜索**：中文 bigram 分词 + 整词加权；**整体匹配（搜索词完整出现）置顶，分词部分匹配排后**（层内按评分排序）；下拉卡片（标题 + 面包屑路径 + 高亮摘要），Ctrl+K 聚焦
